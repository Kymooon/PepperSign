...
/**
 * Servlet che accoda un comando di tipo SAY per Pepper.
 */
@WebServlet("/api/pepper/say")
public class PepperSayServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws IOException {
        ...
        boolean queued = PepperCommandBus.getInstance().enqueue(PepperCommand.say(text));
        int queueSize = PepperCommandBus.getInstance().size();

        if (!queued) {
            log("Impossibile accodare il comando SAY. Coda piena o comando non valido.");
    ...
    /**
     * Per compatibilità, mantiene la GET equivalente alla POST.
     * Se vuoi una semantica HTTP più rigorosa, puoi cambiarla in 405.
     */
    @Override
